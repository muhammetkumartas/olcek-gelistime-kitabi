# ==============================================================================
# ÖLÇEK 3: Psikolojik Dayanıklılık Ölçeği
# Tam Psikometrik Analiz Pipeline'ı
# ==============================================================================
# Faktörler : PD (Pozitif Duygu Düzenleme)   → PD1–PD7
#             MO (Motivasyon & Öz-Yeterlik)   → MO1–MO7
#             SD (Sosyal Destek Algısı)        → SD1–SD7
# Başlangıç : 21 madde, 5'li Likert, N=624 ham
# Ters kodlu: MO3, MO6, SD7
# ==============================================================================
...
set.seed(42)
GECERLI_KOD   <- 1:5
TERS_MADDELER <- c("MO3", "MO6", "SD7")
EKSIK_ESIK    <- 0.10
STRAIGHT_SD   <- 0.10
YAS_MIN <- 18; YAS_MAX <- 60
MAHA_P  <- 0.001
YUK_MIN <- 0.32
DELTA_CAPRAZ <- 0.10
N_FAKTOR <- 3
...
# ── 2. VERİ OKUMA ────────────────────────────────────────────────────────────
df <- read.csv("data/psikolojik_dayaniklilik_ham.csv")
cat(sprintf("\n=== ADIM 1: VERİ OKUMA ===\nHam veri: %d satir x %d sutun\n",
            nrow(df), ncol(df)))
print(head(df, 3))
cat("\nDeğişken tipleri:\n"); print(sapply(df, class))
cat("\nDemografik değişken kodları:\n")
cat("Cinsiyet: 1=Kadın, 2=Erkek\n")
cat("Eğitim  : 1=İlköğretim, 2=Lise, 3=Lisans, 4=Lisansüstü\n")
cat("Medeni  : 1=Bekar, 2=Evli, 3=Boşanmış, 4=Dul\n")
cat("Meslek  : 1=Öğrenci, 2=Memur, 3=Özel Sektör, 4=Serbest, 5=Diğer\n")
...
mes_hata <- !is.na(df$Meslek)       & !df$Meslek %in% 1:5
cat(sprintf("Yas: %d | Medeni: %d | Meslek: %d aykırı → NA yapıldı\n",
            sum(yas_hata), sum(med_hata), sum(mes_hata)))
df$Yas[yas_hata]          <- NA
df$Medeni_Durum[med_hata] <- NA
df$Meslek[mes_hata]       <- NA
...
for (m in MADDE_COLS) {
  ...
  if (any(hata)) {
    df[[m]][hata] <- NA
    toplam_aralik <- toplam_aralik + sum(hata)
  }
}
cat(sprintf("Madde aralık hatası: %d adet NA yapıldı\n", toplam_aralik))

satir_sd <- apply(df[, MADDE_COLS], 1, function(x) sd(x, na.rm = TRUE))
sl_idx   <- which(!is.na(satir_sd) & satir_sd < STRAIGHT_SD)
cat(sprintf("Straight-liner (satır SD < %.2f): %d kişi\n", STRAIGHT_SD, length(sl_idx)))
...
if(length(yuksek)>0) {
  cat(sprintf(">%%10 madde: %s çıkarıldı\n", paste(yuksek,collapse=",")))
  MADDE_COLS <- setdiff(MADDE_COLS, yuksek)
}

satir_eksik <- rowMeans(is.na(df[, MADDE_COLS]))
df <- df[satir_eksik <= EKSIK_ESIK, ]
cat(sprintf("Satır filtresi sonrası N = %d\n", nrow(df)))

df_mice <- df[, c("Yas","Cinsiyet","Egitim","Medeni_Durum","Meslek",MADDE_COLS)]
...
# Ortalama profil grafiği (3 alt boyut için)
p_prof <- tanimlayici %>%
  dplyr::mutate(AltBoyut=substr(Madde,1,2)) %>%
  ggplot(aes(x=Madde, y=Ort, color=AltBoyut, group=AltBoyut)) +
  geom_point(size=4) + geom_line(linewidth=1) +
  geom_errorbar(aes(ymin=Ort-SS, ymax=Ort+SS), width=0.3) +
  geom_hline(yintercept=3, linetype="dashed", color="gray50") +
  scale_color_manual(values=c(PD=MAVI,MO=TUR,SD=YESIL)) +
  scale_y_continuous(limits=c(1,5), breaks=1:5) +
  labs(title="Şekil 2. Madde Profil Grafiği (Ortalama ± 1 SS)",
       x=NULL, y="Ortalama", color="Alt Boyut") +
  theme_minimal(base_size=12) +
  theme(axis.text.x=element_text(angle=45, hjust=1))
ggsave("figures/sekil02_profil.png", p_prof, width=10, height=5, dpi=150)
...
# Histogram matrisi
png("figures/sekil03_histogramlar.png", width=1200, height=900, res=120)
par(mfrow=c(3,7), mar=c(3,2,2,1))
renk_hist <- c(rep(MAVI,7), rep(TUR,7), rep(YESIL,7))
for (i in seq_along(MADDE_COLS))
  hist(df_tc[[MADDE_COLS[i]]], breaks=5, col=renk_hist[i], border="white",
       main=MADDE_COLS[i], xlab="", las=1, xlim=c(1,5))
dev.off()
cat("-> figures/sekil03_histogramlar.png kaydedildi\n")
...
p_maha <- ggplot(data.frame(i=1:length(maha_dist),D2=maha_dist,Ak=maha_p<MAHA_P),
  ...
  labs(title="Şekil 4. Mahalanobis D² Saçılım Grafiği",
       x="Katılımcı İndeksi",y="D²") +
  theme_minimal(base_size=12)
ggsave("figures/sekil04_mahalanobis.png", p_maha, width=8, height=5, dpi=150)
...
kmo_yorum <- as.character(cut(kmo$MSA,c(0,.50,.60,.70,.80,.90,1),
             c("Kabul Edilemez","Yetersiz","Orta","İyi","Çok İyi","Mükemmel"),right=FALSE))
cat(sprintf("KMO: %.3f [%s]  |  Bartlett: χ²(%d)=%.2f, p<.001\n",
            kmo$MSA, kmo_yorum, bart$df, bart$chisq))
cat("\nMadde MSA'ları:\n"); print(round(sort(kmo$MSAi), 3))

# Korelasyon ısı haritası
renk_eksen <- c(rep(MAVI,sum(ALT_BOYUT$PD %in% MADDE_COLS)),
                rep(TUR, sum(ALT_BOYUT$MO %in% MADDE_COLS)),
                rep(YESIL,sum(ALT_BOYUT$SD %in% MADDE_COLS)))
png("figures/sekil05_korelasyon_matrisi.png", width=900, height=820, res=120)
corrplot::corrplot(kor_mat, method="color", type="lower",
                   tl.col=renk_eksen, tl.cex=0.78, diag=FALSE,
                   col=corrplot::COL2("RdBu",200),
                   title="Şekil 5. Korelasyon Matrisi — Alt Boyutlar Renkli",
                   mar=c(0,0,2,0))
legend("bottomright", legend=c("PD","MO","SD"), pch=15, bty="n",
       col=c(MAVI,TUR,YESIL), pt.cex=2, text.col="black")
dev.off()
...
p_scree <- ggplot(head(oz_df,12), aes(x=Faktor,y=Ozd)) +
  geom_line(color="#333",linewidth=1) + geom_point(aes(color=Ozd>=1),size=5) +
  ...
  labs(title="Şekil 6. Scree Plot — Psikolojik Dayanıklılık",
       x="Faktör Sırası",y="Özdeğer (λ)") +
  theme_minimal(base_size=12)
ggsave("figures/sekil06_scree_plot.png", p_scree, width=7, height=5, dpi=150)

png("figures/sekil07_paralel_analiz.png", width=800, height=600, res=120)
pa <- psych::fa.parallel(df_tc[,MADDE_COLS], n.iter=1000, fm="pa", fa="fa",
                          main="Şekil 7. Paralel Analiz", error.bars=TRUE)
dev.off()
...
p_kom <- ggplot(data.frame(Madde=names(kom_vek),h2=round(kom_vek,3),
                             D=ifelse(kom_vek>=.40,"≥.40","<.40"),K=madde_karar$Karar),
  ...
  labs(title="Şekil 8. Madde Komünaliteleri (h²)",x=NULL,y="h²") +
  theme_minimal(base_size=12)
ggsave("figures/sekil08_komunaliteler.png", p_kom, width=7, height=7, dpi=150)
...
p_yuk <- as.data.frame(unclass(fa_rafine$loadings)) %>%
  ...
  labs(title="Şekil 9. Rafine AFA Faktör Yük Isı Haritası",x="Faktör",y=NULL) +
  theme_minimal(base_size=11)
ggsave("figures/sekil09_faktor_yuk_isisi.png", p_yuk, width=6, height=8, dpi=150)
...
p_alpha <- ggplot(guv_df, aes(x=AltBoyut,y=Cronbach_a,
                               ymin=GA_Alt,ymax=GA_Ust,color=AltBoyut)) +
  geom_point(size=7) + geom_errorbar(width=0.18,linewidth=1.3) +
  ...
  scale_color_manual(values=c(PD=MAVI,MO=TUR,SD=YESIL)) +
  ylim(.50,1.10) +
  labs(title="Şekil 10. Alt Ölçek Güvenirlik Katsayıları (%95 GA)",
       x=NULL,y="Cronbach α") +
  theme_minimal(base_size=12)+theme(legend.position="none")
ggsave("figures/sekil10_guvenirlik.png", p_alpha, width=7, height=5, dpi=150)

# ── 15. DFA ──────────────────────────────────────────────────────────────────
cat("\n=== ADIM 14: DOĞRULAYICI FAKTÖR ANALİZİ ===\n")
set.seed(2024)
idx2    <- sample(nrow(df_tc), floor(nrow(df_tc)/2))
df_dfa  <- df_tc[-idx2, ]
cat(sprintf("DFA n = %d\n", nrow(df_dfa)))

pd_kal <- intersect(ALT_BOYUT$PD, kalacak)
mo_kal <- intersect(ALT_BOYUT$MO, kalacak)
sd_kal <- intersect(ALT_BOYUT$SD, kalacak)

model_dfa <- sprintf("PD =~ %s\nMO =~ %s\nSD =~ %s",
                     paste(pd_kal,collapse=" + "),
                     paste(mo_kal,collapse=" + "),
                     paste(sd_kal,collapse=" + "))
cat(model_dfa,"\n\n")
...
yuk_tab <- std_coz[std_coz$op=="=~", c("lhs","rhs","est.std","se","z","pvalue")]
names(yuk_tab) <- c("Faktör","Madde","β","SE","z","p")
...
ave_cr_df <- dplyr::bind_rows(lapply(
  list(PD=pd_kal, MO=mo_kal, SD=sd_kal), function(mds) {
...
cat("\nHTMT (< .85 kabul | < .90 maksimum):\n"); print(round(htmt_res,3))

fak_cors <- lavaan::lavInspect(fit_dfa,"cor.lv")
cat("\nFornell-Larcker kontrolü:\n")
for (nm in ave_cr_df$Faktör) {
  dig <- ave_cr_df$sqAVE[ave_cr_df$Faktör==nm]
  oth <- fak_cors[nm, rownames(fak_cors)[rownames(fak_cors)!=nm]]
  cat(sprintf("  %s: √AVE=%.3f > max(r)=%.3f  %s\n",
              nm, dig, max(abs(oth)), if(dig>max(abs(oth))) "✓" else "⚠"))
}

# ── 17. NİHAİ PUANLAMA ───────────────────────────────────────────────────────
cat("\n=== ADIM 16: NİHAİ PUANLAMA ===\n")
df_son <- df_tc[,c("ID","Yas","Cinsiyet","Meslek",kalacak)]
df_son$Puan_PD     <- rowMeans(df_son[,pd_kal],na.rm=TRUE)
df_son$Puan_MO     <- rowMeans(df_son[,mo_kal],na.rm=TRUE)
df_son$Puan_SD     <- rowMeans(df_son[,sd_kal],na.rm=TRUE)
df_son$Puan_Toplam <- rowMeans(df_son[,c("Puan_PD","Puan_MO","Puan_SD")])
cat("\nAlt ölçek özeti:\n")
print(sapply(df_son[,c("Puan_PD","Puan_MO","Puan_SD","Puan_Toplam")],
             function(x) c(Ort=round(mean(x),3),SS=round(sd(x),3),
                           Min=round(min(x),2),Max=round(max(x),2))))

# Puan dağılım + meslek karşılaştırması
p_dist <- df_son %>%
  dplyr::select(ID,Puan_PD,Puan_MO,Puan_SD,Puan_Toplam) %>%
  ...
  scale_fill_manual(values=c(Puan_PD=MAVI,Puan_MO=TUR,
                              Puan_SD=YESIL,Puan_Toplam=MOR)) +
  labs(title="Şekil 11. Alt Ölçek Puan Dağılımları",
       x="Puan (1–5)",y="Yoğunluk") +
  theme_minimal(base_size=11)+theme(legend.position="none")
ggsave("figures/sekil11_puan_dagilimi.png", p_dist, width=8, height=6, dpi=150)

# Meslek grupları arası karşılaştırma
meslek_lab <- c("1"="Öğrenci","2"="Memur","3"="Özel","4"="Serbest","5"="Diğer")
p_mes <- df_son %>%
  dplyr::mutate(Meslek_lab=dplyr::recode(as.character(Meslek), !!!meslek_lab)) %>%
  dplyr::select(Meslek_lab,Puan_PD,Puan_MO,Puan_SD) %>%
  tidyr::pivot_longer(-Meslek_lab,names_to="Boyut",values_to="Puan") %>%
  dplyr::group_by(Meslek_lab,Boyut) %>%
  dplyr::summarise(Ort=mean(Puan,na.rm=TRUE), .groups="drop") %>%
  ggplot(aes(x=Meslek_lab,y=Ort,fill=Boyut)) +
  geom_col(position="dodge",width=0.7) +
  geom_hline(yintercept=3,linetype="dashed",color="gray40") +
  scale_fill_manual(values=c(Puan_PD=MAVI,Puan_MO=TUR,Puan_SD=YESIL)) +
  labs(title="Şekil 12. Meslek Gruplarına Göre Alt Ölçek Ortalamaları",
       x=NULL,y="Ortalama",fill="Alt Boyut") +
  ylim(0,5) + theme_minimal(base_size=11)
ggsave("figures/sekil12_meslek_karsilastirma.png", p_mes, width=9, height=5, dpi=150)
...
ws("Puanlar",      df_son[,c("ID","Puan_PD","Puan_MO","Puan_SD","Puan_Toplam")])
openxlsx::saveWorkbook(wb,"output/psikolojik_dayaniklilik_analiz_sonuclari.xlsx",overwrite=TRUE)
cat("Excel: output/psikolojik_dayaniklilik_analiz_sonuclari.xlsx (10 sekme)\n")
cat("\n============================================================\n")
cat("  Pipeline tamamlandi — Psikolojik Dayaniklilik Ölçegi\n")
cat("============================================================\n")
